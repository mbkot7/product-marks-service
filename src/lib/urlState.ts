...
export async function createGistShareLink(marks: ProductMarkDetail[]): Promise<string> {
  ...
  const gistData: GistData = {
    ...
    files: {
      ...
      'README.md': {
        content: `# Product Marks Data

This Gist contains product marks data exported from the Product Marks Service.

- **Marks Count**: ${marks.length}
- **Created**: ${new Date(timestamp).toLocaleString()}
- **Version**: ${metadata.version}

## Usage

Open this data in the Product Marks Service by visiting the application with this Gist ID.
...
Generated by [Product Marks Service](https://github.com/mbkot7/product-marks-service)
`
      }
    }
  };
